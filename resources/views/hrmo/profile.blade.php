    ...
    <section class="mx-6 mt-6">
        <div class="rounded-[40px] bg-white p-10 shadow-gray-400 ring-1 ring-slate-100">
            ...
            <div class="mt-10 flex flex-wrap items-center justify-center gap-4 rounded-[32px] bg-slate-50 p-6 ring-1 ring-slate-100">
                ...
                <!-- HRMO Account -->
                <button type="button" onclick="openEditHRMOModal()" class="rounded-2xl bg-blue-100 px-6 py-3 text-base font-bold text-blue-700 transition hover:bg-blue-200">
                    Update HRMO Profile
                </button>

                <!-- Archive -->
                <button type="button" onclick="openRequestArchiveAccountModal()" class="rounded-2xl bg-red-100 px-6 py-3 text-base font-bold text-red-700 transition hover:bg-red-200">
                    Archive Account
                </button>
...
<x-modals.edit-profile :agency="$agency" :fieldOffices="$field_offices" />
    ...
    <x-modals.edit-hrmo-profile :user="$user" />

    <x-modals.request-acc-archive :user="$user" />
...
<script>
    ...
    function openEditHRMOModal(){
        document.getElementById('editHRMOModal').classList.remove('hidden');
        document.getElementById('editHRMOModal').classList.add('flex');
    }
    function closeEditHRMOModal(){
        document.getElementById('editHRMOModal').classList.add('hidden');
        document.getElementById('editHRMOModal').classList.remove('flex');

    }

    function openRequestArchiveAccountModal(){
        document.getElementById('requestArchiveAccountModal').classList.remove('hidden');
        document.getElementById('requestArchiveAccountModal').classList.add('flex');
    }
    function closeRequestArchiveAccountModal(){
        document.getElementById('requestArchiveAccountModal').classList.add('hidden');
        document.getElementById('requestArchiveAccountModal').classList.remove('flex');
